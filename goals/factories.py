...
class GoalFactory(DjangoModelFactory):
    ...
    goal_type = factory.Faker('random_element', elements=[
...
class BodyMeasurementFactory(DjangoModelFactory):
    ...
    user = factory.SubFactory(UserFactory)
    metric = factory.Faker('random_element', elements=[
        'weight_kg', 'waist_cm', 'hip_cm', 'neck_cm',
        'arm_circumference_cm', 'thigh_circumference_cm', 
        'calf_circumference_cm', 'body_fat_percentage',
        'muscle_mass_percentage', 'bmi_value'
    ])
